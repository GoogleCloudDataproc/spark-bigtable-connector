  ...
  private def convertTypeToAvro[T](
    ...
    dataType match {
      ...
      case ArrayType(elementType, _) =>
        ...
        val elementSchema =
          convertTypeToAvro(elementType, builder, structName, recordNamespace)
      ...
      case MapType(StringType, valueType, _) =>
        ...
        val valueSchema =
          convertTypeToAvro(valueType, builder, structName, recordNamespace)
...
object AvroSerdes {
  ...
  def serialize(input: Any, schema: Schema): Array[Byte] = {
    schema.getType match {
      case BOOLEAN       => BytesConverter.toBytes(input.asInstanceOf[Boolean])
      case BYTES | FIXED => input.asInstanceOf[Array[Byte]]
      case DOUBLE        => BytesConverter.toBytes(input.asInstanceOf[Double])
      case FLOAT         => BytesConverter.toBytes(input.asInstanceOf[Float])
      case INT           => BytesConverter.toBytes(input.asInstanceOf[Int])
      case LONG          => BytesConverter.toBytes(input.asInstanceOf[Long])
      case STRING        => BytesConverter.toBytes(input.asInstanceOf[String])
      case RECORD =>
  ...
  def deserialize(input: Array[Byte], schema: Schema): GenericRecord = {
    val reader2: DatumReader[GenericRecord] =
