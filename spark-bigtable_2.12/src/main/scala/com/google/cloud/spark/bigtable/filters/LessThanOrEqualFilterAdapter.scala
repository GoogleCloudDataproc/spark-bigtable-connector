...
object LessThanOrEqualFilterAdapter {
    ...
    value match {
      ...
      case typedVal: Long =>
        ...
        if (typedVal >= 0) {
          rangeSetResult.add(
            Range.closed(
              new RowKeyWrapper(BytesConverter.toBytes(0: Long)),
              new RowKeyWrapper(valueBytes)
            )
          )
          rangeSetResult.add(
            Range.closed(
              new RowKeyWrapper(BytesConverter.toBytes(Long.MinValue)),
              new RowKeyWrapper(BytesConverter.toBytes(-1: Long))
            )
          )
        } else {
          rangeSetResult.add(
